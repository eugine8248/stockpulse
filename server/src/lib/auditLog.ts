// Auth audit log helper. Fire-and-forget so the writer never blocks the
// response — if the DB write fails we log it and move on.
...
function ipFromReq(req: Request): string {
  // Express may return ::ffff:127.0.0.1 — that's fine, we don't normalise.
